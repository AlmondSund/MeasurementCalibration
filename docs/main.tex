...
\subsection{Indices, data, and preprocessing}
...
The implemented preprocessing stage does the following.

\begin{enumerate}
    \item It parses one acquisition CSV per sensor and extracts the PSD vectors.
    \item It selects the frequency band shared by all sensors.
    \item It aligns the experiment sequences by searching a small integer shift for each sensor relative to a designated reference sensor.
    \item It constructs a uniform frequency grid from the selected common band.
    \item It interpolates each sensor's nominal response CSV onto that grid.
    \item It normalizes the nominal gains so that their geometric mean is one at each frequency.
\end{enumerate}

Denote the resulting nominal gain by $G^{(0)}_{s,j}>0$. The normalization is
\begin{equation}
\bar G^{(0)}_j
=
\exp\!\left(
...
\right),
\qquad
G^{(0)}_{s,j}\leftarrow \frac{G^{(0)}_{s,j}}{\bar G^{(0)}_j}.
\label{eq:nominal-normalization}
\end{equation}

Because the loader constructs equally spaced frequency centers on the retained band, the second-difference smoothers used later act on a uniform grid.
...
\label{eq:ident-full}
\end{equation}
because the nominal gains have already been normalized by \eqref{eq:nominal-normalization}.
...
Let $s_\star$ denote the sensor believed to be the most reliable one. The implementation does \emph{not} impose a hard reference constraint. Instead it uses two soft mechanisms:
...
\begin{enumerate}
    ...
    \]
    meaning that the reliable sensor should stay close to its nominal curve.
...
Let $D_2\in\mathbb R^{(J-2)\times J}$ denote the second-difference operator
\[
(D_2 x)_r = x_r - 2x_{r+1} + x_{r+2},
\qquad r=1,\dots,J-2.
\]
The implementation uses the corresponding Gram matrix
\begin{equation}
L = D_2^\top D_2.
\label{eq:L}
\end{equation}
...
\begin{enumerate}
    \item preprocess and align the common-field dataset;
    \item normalize nominal gains to the network reference scale;
